...
(deftest single-input-output
  (test-graph-events
    [{:inputs  [[:a]]
      :outputs [[:b]]
      :handler (fn [ctx {:keys [a]} {:keys [b]}] [(+ a b)])}]
    [[[:a] 1]]
...
(deftest single-unchanged-input
  ...
  (test-graph-events
    [{:inputs  [[:a]]
      :outputs [[:b]]
      :handler (fn [ctx {:keys [a]} {:keys [b]}] [(inc a)])}]
    [[[:a] 0]]
...
(deftest cyclic-inputs
  (test-graph-events
    [{:inputs  [[:a]]
      :outputs [[:b] [:c]]
      :handler (fn [ctx {:keys [a]} {:keys [b c]}] [(inc b) c])}
     {:inputs  [[:b]]
      :outputs [[:a]]
      :handler (fn [ctx {:keys [b]} {:keys [a]}] [(inc a)])}]
    [[[:a] 1]]
  ...
  (test-graph-events
    [{:inputs  [[:a]]
      :outputs [[:b] [:c]]
      :handler (fn [ctx {:keys [a]} {:keys [b c]}] [(+ a b) c])}
     {:inputs  [[:b]]
      :outputs [[:a]]
      :handler (fn [ctx {:keys [b]} {:keys [a]}] [(+ b a)])}]
    [[[:a] 1] [[:b] 2]]
...
(deftest test-cascading-events
  (test-graph-events
    [{:inputs  [[:a]]
      :outputs [[:b] [:c]]
      :handler (fn [ctx {:keys [a]} {:keys [b c]}] [(+ a b) (+ a c)])}
     {:inputs  [[:c]]
      :outputs [[:d]]
      :handler (fn [ctx {:keys [c]} _] [(inc c)])}]
    [[[:a] 1] [[:b] 1]]
...
(deftest multi-input-event
  (test-graph-events
    [{:inputs  [[:a] [:b]]
      :outputs [[:c]]
      :handler (fn [ctx {:keys [a b]} {:keys [c]}] [(+ a b)])}]
    [[[:a] 1] [[:b] 1]]
...
(deftest multi-output-event
  (test-graph-events
    [{:inputs  [[:a]]
      :outputs [[:b] [:c]]
      :handler (fn [ctx {:keys [a]} {:keys [b c]}] [(+ a b) (inc c)])}]
    [[[:a] 1]]
...
(deftest multi-input-output-event
  (test-graph-events
    [{:inputs  [[:a] [:b]]
      :outputs [[:b] [:c] [:d]]
      :handler (fn [ctx {:keys [a b]} {:keys [c d]}] [(+ a b) c d])}]
    [[[:a] 1] [[:b] 1]]
...
(deftest context-access
  (test-graph-events
    ...
    [{:inputs  [[:a]]
      :outputs [[:b]]
      :handler (fn [ctx {:keys [a]} _] [((:action ctx) a)])}]
    [[[:a] 1]]
...
(deftest triggering-sub-path
  (test-graph-events
    [{:inputs  [[:h]]
      :outputs [[:h]]
      :handler (fn [ctx {:keys [h]} {:keys [old-h]}]
                 [(update old-h :i + (:i h))])}]
    [[[:h :i] 1]]                                           ;; [[[:h] {:i 2}]]
